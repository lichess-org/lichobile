...
import { transformProp } from '../ui/helper'
  ...
  return Vnode(
    ...
    undefined,
    {
      ...
      oncreate: function(vnode) {
        ctrl.data.transformProp = transformProp()
        ...
        if (!ctrl.data.viewOnly) {
          ...
          shadow.style[ctrl.data.transformProp] = util.translate3dAway
        ...
        if (!ctrl.data.viewOnly && ctrl.data.draggable.showGhost) {
          ...
          ghost.style[ctrl.data.transformProp] = util.translateAway
  ...
  while (el) {
    ...
    if (el.tagName === 'PIECE') {
      ...
      if (el.cgDragging && d.draggable.current.orig !== k) {
        ...
        el.style[d.transformProp] = util.transform(d, el.cgColor, util.translate(translate))
        el.cgDragging = false
      }
  ...
  for (let j = 0, jlen = piecesKeys.length; j < jlen; j++) {
    ...
    if (!samePieces.has(k)) {
      ...
      if (mvd) {
        ...
        mvd.style[d.transformProp] = util.transform(d, mvd.cgColor, util.translate(translate))
      }
      // no piece in moved obj: insert the new piece
        ...
        pe.style[d.transformProp] = util.transform(d, pe.color, util.translate(translate))
        ctrl.data.element.appendChild(pe)
      }
    }
  }
  ...
  squares.forEach((v, k) => {
    if (!sameSquares.has(k)) {
      ...
      if (mvd) {
        mvd.cgKey = k
        translate = util.posToTranslate(util.key2pos(k), asWhite, bounds)
        mvd.style[d.transformProp] = util.translate(translate)
      }
      else {
        const se = document.createElement('square')
        se.className = v
        se.cgKey = k
        se.style[d.transformProp] = util.translate(util.posToTranslate(util.key2pos(k), asWhite, bounds))
        ctrl.data.element.appendChild(se)
      }
    }
  })
