...
module.exports = function(ctrl) {
  function renderGame(ctrl){
    return m('div', [
        ...
        m('button', { config: function(el, isUpdate) {
          if (!isUpdate) el.addEventListener('touchstart', ctrl.vm.startAiGame);
        }}, 'Start!')
    ]);
  }
  ...
  function renderPlayer(ctrl){
    ...
    if (ctrl.vm.clocks.player) children.push(clock.view(ctrl.vm.clocks.player), m('div.timer.after'));
  ...
  function renderOpponent(ctrl){
    ...
    if (ctrl.vm.clocks.opponent) children.push(clock.view(ctrl.vm.clocks.opponent), m('div.timer.before'));
