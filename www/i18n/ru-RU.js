export default {
  "playWithAFriend": "Сыграть с другом",
  "playWithTheMachine": "Сыграть с компьютером",
  "toInviteSomeoneToPlayGiveThisUrl": "Чтобы пригласить друга, поделитесь с ним этой ссылкой",
  "gameOver": "Игра завершена",
  "waitingForOpponent": "Ожидание соперника",
  "waiting": "Ожидание",
  "yourTurn": "Ваш ход",
  "aiNameLevelAiLevel": "%1$s уровня %2$s",
  "level": "Уровень",
  "toggleTheChat": "Показывать окно чата",
  "toggleSound": "Вкл/выкл звук",
  "chat": "Чат",
  "resign": "Сдаться",
  "checkmate": "Мат",
  "stalemate": "Пат",
  "white": "Белые",
  "black": "Чёрные",
  "randomColor": "Случайный цвет",
  "createAGame": "Создать игру",
  "whiteIsVictorious": "Победа белых",
  "blackIsVictorious": "Победа чёрных",
  "kingInTheCenter": "Король в центре",
  "threeChecks": "Три шаха",
  "raceFinished": "Гонка окончена",
  "variantEnding": "Вариация завершена",
  "newOpponent": "Новый соперник",
  "yourOpponentWantsToPlayANewGameWithYou": "Ваш соперник предлагает вам сыграть ещё раз",
  "joinTheGame": "Присоединиться",
  "whitePlays": "Ход белых",
  "blackPlays": "Ход чёрных",
  "opponentLeftChoices": "Ваш соперник покинул игру. Вы можете запросить победу, объявить ничью или подождать.",
  "makeYourOpponentResign": "Объявить соперника сдавшимся",
  "forceResignation": "Запросить победу",
  "forceDraw": "Объявить ничью",
  "talkInChat": "Будьте вежливы в чате!",
  "theFirstPersonToComeOnThisUrlWillPlayWithYou": "Первый, кто перейдёт по этой ссылке и присоединится к игре, будет играть с Вами.",
  "whiteResigned": "Белые сдались",
  "blackResigned": "Чёрные сдались",
  "whiteLeftTheGame": "Белые вышли из игры",
  "blackLeftTheGame": "Чёрные вышли из игры",
  "shareThisUrlToLetSpectatorsSeeTheGame": "Пригласите зрителей, поделившись с ними этой ссылкой",
  "theComputerAnalysisHasFailed": "Произошёл сбой.",
  "viewTheComputerAnalysis": "Просмотреть компьютерный анализ",
  "requestAComputerAnalysis": "Запросить компьютерный анализ",
  "computerAnalysis": "Компьютерный анализ",
  "computerAnalysisAvailable": "Доступен компьютерный анализ",
  "analysis": "Анализ",
  "depthX": "Глубина %s",
  "usingServerAnalysis": "Используя компьютерный анализ",
  "loadingEngine": "Загрузка движка...",
  "cloudAnalysis": "Облачный анализ",
  "goDeeper": "Глубже",
  "showThreat": "Показывать опасные моменты",
  "inLocalBrowser": "на компьютере",
  "toggleLocalEvaluation": "Включить анализ на компьютере",
  "move": "Ход",
  "variantLoss": "Шанс проигрыша",
  "variantWin": "Шанс выигрыша",
  "insufficientMaterial": "Недостаточно информации",
  "pawnMove": "Ход пешкой",
  "capture": "Захвачен",
  "close": "Закрыть",
  "winning": "Выигрыш",
  "losing": "Проигрыш",
  "drawn": "Ничья",
  "unknown": "Неизвестно",
  "database": "Данные из",
  "whiteDrawBlack": "Белые / ничья / черные",
  "averageRatingX": "Средняя оценка: %s",
  "recentGames": "Последние игры",
  "topGames": "Лучшие игры",
  "masterDbExplanation": "Два миллиона OTB игр %1$s+ FIDE игроков в рейтинге от %2$s до %3$s",
  "noGameFound": "Игр не найдено",
  "maybeIncludeMoreGamesFromThePreferencesMenu": "Может, стоит включить больше игр из вкладки предпочтений?",
  "openingExplorer": "Анализ дебютных вариантов",
  "xOpeningExplorer": "Открываем проводник: %s",
  "winPreventedBy50MoveRule": "Победа предотвращена правилом 50-го хода",
  "lossSavedBy50MoveRule": "Поражение спасено правилом 50-го хода",
  "allSet": "Все готово!",
  "importPgn": "Импортировать PGN",
  "delete": "Удалить",
  "deleteThisImportedGame": "Удалить эту импортированную игру?",
  "replayMode": "Режим повтора",
  "realtimeReplay": "В реальном времени",
  "byCPL": "Интересное",
  "openStudy": "Начать исследование",
  "enable": "Включить",
  "bestMoveArrow": "Стрелка \"лучший ход\\\"",
  "evaluationGauge": "Шкала оценки",
  "multipleLines": "Количество стрелок",
  "cpus": "Процессоры",
  "memory": "Память",
  "infiniteAnalysis": "Бесконечный анализ",
  "removesTheDepthLimit": "Удаляет ограничение на глубину (ваш компьютер будет горячим)",
  "blunders": "Грубые ошибки",
  "mistakes": "Ошибки",
  "inaccuracies": "Сомнительные ходы",
  "moveTimes": "Время ходов",
  "flipBoard": "Перевернуть доску",
  "threefoldRepetition": "Троекратное повторение ходов",
  "claimADraw": "Объявить ничью",
  "offerDraw": "Предложить ничью",
  "draw": "Ничья",
  "currentGames": "Текущие партии",
  "viewInFullSize": "Посмотреть в полном размере",
  "logOut": "Выйти",
  "signIn": "Войти",
  "newToLichess": "Впервые на Lichess?",
  "youNeedAnAccountToDoThat": "Вам нужно зарегистрироваться, чтобы сделать это",
  "signUp": "Регистрация",
  "computersAreNotAllowedToPlay": "Создание учётных записей для компьютерных программ запрещено. Пожалуйста, во время игры не используйте шахматных ботов, базы данных или подсказки от других игроков.",
  "games": "Игры",
  "forum": "Форум",
  "xPostedInForumY": "%1$s опубликовал на форуме %2$s",
  "latestForumPosts": "Последние сообщения на форуме",
  "players": "Игроки",
  "minutesPerSide": "Минут на партию",
  "variant": "Вариант",
  "variants": "Варианты",
  "timeControl": "Контроль времени",
  "realTime": "В реальном времени",
  "correspondence": "По переписке",
  "daysPerTurn": "Дней на ход",
  "oneDay": "Один день",
  "time": "Время",
  "rating": "Рейтинг",
  "ratingStats": "Статистика рейтинга",
  "username": "Имя пользователя",
  "usernameOrEmail": "Логин или электронная почта",
  "password": "Пароль",
  "haveAnAccount": "Уже есть учётная запись?",
  "changePassword": "Сменить пароль",
  "changeEmail": "Изменить электронную почту",
  "email": "Электронная почта",
  "emailIsOptional": "Указывать электронную почту не обязательно. Lichess будет использовать её для сброса пароля, если Вы его забудете.",
  "passwordReset": "Сброс пароля",
  "forgotPassword": "Забыли пароль?",
  "rank": "Уровень",
  "rankX": "Рейтинг: %s",
  "rankIsUpdatedEveryXMinutes": "Рейтинг обновляется каждые %s минут",
  "gamesPlayed": "Партий сыграно",
  "cancel": "Отменить",
  "timeOut": "Время истекло",
  "drawOfferSent": "Предложение ничьей отправлено",
  "drawOfferDeclined": "Предложение ничьей отклонено",
  "drawOfferAccepted": "Предложение ничьей принято",
  "drawOfferCanceled": "Предложение ничьей отменено",
  "whiteOffersDraw": "Белые предлагают ничью",
  "blackOffersDraw": "Чёрные предлагают ничью",
  "whiteDeclinesDraw": "Белые отклонили предложение ничьей",
  "blackDeclinesDraw": "Чёрные отклонили предложение ничьей",
  "yourOpponentOffersADraw": "Ваш оппонент предлагает ничью",
  "accept": "Принять",
  "decline": "Отклонить",
  "playingRightNow": "Активная партия",
  "finished": "Закончено",
  "abortGame": "Отменить игру",
  "gameAborted": "Игра отменена",
  "standard": "Классические шахматы",
  "unlimited": "Без лимита времени",
  "mode": "Режим",
  "casual": "Нерейтинговая",
  "rated": "Рейтинговая",
  "thisGameIsRated": "Игра на рейтинг",
  "rematch": "Реванш",
  "rematchOfferSent": "Предложение реванша отправлено",
  "rematchOfferAccepted": "Предложение реванша принято",
  "rematchOfferCanceled": "Предложение реванша отменено",
  "rematchOfferDeclined": "Предложение реванша отклонено",
  "cancelRematchOffer": "Отказаться от реванша",
  "viewRematch": "Посмотреть матч-реванш",
  "play": "Играть",
  "inbox": "Входящие",
  "chatRoom": "Окно чата",
  "loginToChat": "Войдите, чтобы использовать чат",
  "youHaveBeenTimedOut": "Вы были отключены.",
  "spectatorRoom": "Чат для зрителей",
  "composeMessage": "Написать сообщение",
  "noNewMessages": "Нет новых сообщений",
  "subject": "Тема",
  "recipient": "Получатель",
  "send": "Послать",
  "incrementInSeconds": "Прибавка в секундах",
  "freeOnlineChess": "Бесплатные онлайн-шахматы",
  "spectators": "Зрители:",
  "exportGames": "Экспорт игр",
  "ratingRange": "Диапазон рейтинга",
  "giveMoreTime": "Добавить времени",
  "thisPlayerUsesChessComputerAssistance": "Этот игрок использует помощь шахматной программы (читер)",
  "thisPlayerArtificiallyIncreasesTheirRating": "Этот игрок искусственно увеличивал либо уменьшал свой рейтинг",
  "openingExplorerAndTablebase": "Открываем проводник & таблицу базы",
  "takeback": "Вернуть ход",
  "proposeATakeback": "Попросить соперника вернуть ход",
  "takebackPropositionSent": "Предложение возврата отправлено",
  "takebackPropositionDeclined": "Предложение возврата отклонено",
  "takebackPropositionAccepted": "Предложение возврата принято",
  "takebackPropositionCanceled": "Предложение возврата отменено",
  "yourOpponentProposesATakeback": "Ваш оппонент просит вернуть ход",
  "bookmarkThisGame": "Добавить игру в закладки",
  "search": "Поиск",
  "advancedSearch": "Расширенный поиск",
  "tournament": "Турнир",
  "tournaments": "Турниры",
  "tournamentPoints": "Турнирные очки",
  "viewTournament": "Просмотр турнира",
  "backToTournament": "Обратно в турнир",
  "backToGame": "Возврат к игре",
  "siteDescription": "Бесплатные онлайн-шахматы. Играйте в шахматы прямо сейчас в простом интерфейсе без рекламы. Не требует регистрации и скачивания программы. Играйте в шахматы с компьютером, друзьями или случайными соперниками.",
  "teams": "Команды",
  "allTeams": "Все команды",
  "newTeam": "Новая команда",
  "myTeams": "Мои команды",
  "noTeamFound": "Команда не найдена",
  "joinTeam": "Присоединиться к команде",
  "quitTeam": "Выйти из команды",
  "anyoneCanJoin": "Любой может присоединиться",
  "aConfirmationIsRequiredToJoin": "Для присоединения требуется подтверждение",
  "joiningPolicy": "Условия членства",
  "teamLeader": "Лидер команды",
  "teamBestPlayers": "Лучшие игроки команды",
  "teamRecentMembers": "Новые члены команды",
  "xJoinedTeamY": "%1$s присоединился к команде %2$s",
  "xCreatedTeamY": "%1$s создал команду %2$s",
  "averageElo": "Средний рейтинг",
  "location": "Местоположение",
  "settings": "Настройки",
  "filterGames": "Фильтр партий",
  "reset": "Сбросить",
  "apply": "Применить",
  "leaderboard": "Лучшие игроки",
  "pasteTheFenStringHere": "Вставьте строку в формате FEN",
  "pasteThePgnStringHere": "Вставьте строку в формате PGN",
  "fromPosition": "С позиции",
  "continueFromHere": "Продолжить с данного места",
  "importGame": "Импорт игры",
  "importGameExplanation": "При вставке PGN игры вы получаете повтор в браузере,\nкомпьютерный анализ, игровой чат и ссылку, которой можно поделиться.",
  "thisIsAChessCaptcha": "Это шахматная CAPTCHA.",
  "clickOnTheBoardToMakeYourMove": "Кликните по доске, чтобы сделать ход и доказать, что вы человек.",
  "captcha.fail": "Пожалуйста, решите шахматную CAPTCHA.",
  "notACheckmate": "Это не мат",
  "colorPlaysCheckmateInOne": "%s начинают; мат в один ход",
  "retry": "Повторить",
  "reconnecting": "Переподключение",
  "onlineFriends": "Друзья онлайн",
  "noFriendsOnline": "Нет друзей онлайн",
  "findFriends": "Найти друзей",
  "favoriteOpponents": "Предпочтительные соперники",
  "follow": "Подписаться",
  "following": "Подписаны",
  "unfollow": "Отписаться",
  "block": "Заблокировать",
  "blocked": "Заблокированные",
  "unblock": "Разблокировать",
  "followsYou": "Подписаны на вас",
  "xStartedFollowingY": "%1$s подписался на %2$s",
  "more": "Ещё",
  "memberSince": "Дата регистрации",
  "lastSeenActive": "Последнее посещение %s",
  "challengeToPlay": "Вызвать на игру",
  "player": "Игрок",
  "list": "Список",
  "graph": "График",
  "required": "Требуется заполнить.",
  "openTournaments": "Открытые турниры",
  "duration": "Длительность",
  "winner": "Победитель",
  "standing": "Турнирная таблица",
  "createANewTournament": "Создать новый турнир",
  "join": "Присоединиться",
  "withdraw": "Покинуть",
  "points": "Очки",
  "wins": "Победы",
  "losses": "Поражения",
  "winStreak": "Серия побед",
  "createdBy": "Создан",
  "tournamentIsStarting": "Турнир начинается",
  "tournamentPairingsAreNowClosed": "Жеребьёвка турнира в настоящее время закрыта.",
  "standByX": "Ожидайте, %s, жеребьёвка игроков, будьте готовы!",
  "youArePlaying": "Вы в игре!",
  "winRate": "Коэффициент победы",
  "berserkRate": "Коэффициент бешенства",
  "performance": "Эффективность",
  "tournamentComplete": "Соревнование завершено",
  "movesPlayed": "Ходов сыграно",
  "whiteWins": "Белые выиграли",
  "blackWins": "Черные выиграли",
  "draws": "Ничья",
  "nextXTournament": "Следующий %s турнир:",
  "viewMoreTournaments": "Просмотреть больше турниров",
  "averageOpponent": "Средний рейтинг соперника",
  "membersOnly": "Только для зарегистрированных",
  "boardEditor": "Редактор доски",
  "setTheBoard": "Установить доску",
  "popularOpenings": "Популярные дебюты",
  "startPosition": "Начальная позиция",
  "clearBoard": "Очистить доску",
  "savePosition": "Сохранить позицию",
  "loadPosition": "Загрузить позицию",
  "isPrivate": "Закрытый",
  "reportXToModerators": "Сообщить о %s модераторам",
  "profileCompletion": "Заполненность профиля: %s",
  "xRating": "%s рейтинг",
  "ifNoneLeaveEmpty": "Если нет, оставьте пустым",
  "gameCompletionRate": "Завершённых игр: %s",
  "profile": "Профиль",
  "editProfile": "Редактировать профиль",
  "firstName": "Имя",
  "lastName": "Фамилия",
  "biography": "О себе",
  "country": "Страна",
  "preferences": "Настройки",
  "watchLichessTV": "Смотреть Lichess TV",
  "previouslyOnLichessTV": "Ранее на Lichess TV",
  "onlinePlayers": "Игроки в сети",
  "activeToday": "Активные сегодня",
  "activePlayers": "Активные игроки",
  "bewareTheGameIsRatedButHasNoClock": "Внимание, это рейтинговая игра без ограничения по времени!",
  "training": "Задачи",
  "yourPuzzleRatingX": "Ваш рейтинг в решении задач: %s",
  "findTheBestMoveForWhite": "Найдите лучший ход за белых.",
  "findTheBestMoveForBlack": "Найдите лучший ход за чёрных.",
  "toTrackYourProgress": "Для отслеживания прогресса:",
  "trainingSignupExplanation": "Lichess подберет задачи для вашего уровня, что позволит повысить эффективность тренировок.",
  "puzzleId": "Задача %s",
  "puzzleOfTheDay": "Шахматная задача дня",
  "clickToSolve": "Нажмите, чтобы решить",
  "goodMove": "Хороший ход",
  "butYouCanDoBetter": "Но вы можете найти лучше.",
  "bestMove": "Лучший ход!",
  "keepGoing": "Продолжайте...",
  "puzzleFailed": "Задача не решена.",
  "butYouCanKeepTrying": "Но вы можете попробовать ещё раз.",
  "success": "Успех",
  "wasThisPuzzleAnyGood": "Была ли задача интересной?",
  "pleaseVotePuzzle": "Помогите сделать lichess лучше, голосуя при помощи стрелок вверх или вниз.",
  "thankYou": "Спасибо!",
  "ratingX": "Рейтинг %s",
  "fromGameLink": "Из игры %s",
  "continueTraining": "Продолжить тренировку",
  "retryThisPuzzle": "Попробовать ещё раз",
  "thisPuzzleIsCorrect": "Задача правильная и интересная",
  "thisPuzzleIsWrong": "Задача неправильная или неинтересная",
  "automaticallyProceedToNextGameAfterMoving": "Автоматически переходить к следующей игре после хода",
  "autoSwitch": "Автосмена",
  "puzzles": "Задачки",
  "latestUpdates": "Последние обновления",
  "tournamentWinners": "Победители турнира",
  "name": "Имя",
  "description": "Описание",
  "no": "Нет",
  "yes": "Да",
  "help": "Помощь",
  "createANewTopic": "Создать новую тему",
  "topics": "Темы",
  "posts": "Посты",
  "lastPost": "Последний пост",
  "views": "Прочитано",
  "replies": "Ответы",
  "replyToThisTopic": "Принять участие в обсуждении",
  "reply": "Ответ",
  "message": "Сообщение",
  "createTheTopic": "Создать тему",
  "reportAUser": "Пожаловаться на пользователя",
  "user": "Пользователь",
  "reason": "Причина",
  "whatIsIheMatter": "В чём дело?",
  "cheat": "Использование помощи сторонних программ",
  "insult": "Оскорбление",
  "troll": "Троллинг",
  "other": "Другое",
  "reportDescriptionHelp": "Поделитесь с нами ссылкой на игру (игры), где, как вам кажется, были нарушены правила, и опишите, в чем дело. Пожалуйста, расскажите, как и исходя из чего вы догадались, что ваш противник нечестен. Мы сработаем оперативнее, если ваше сообщение будет на английском языке.",
  "by": "опубликовал %s",
  "thisTopicIsNowClosed": "Эта тема сейчас закрыта.",
  "theming": "Настройка темы",
  "donate": "Сделать пожертвование",
  "blog": "Блог",
  "questionsAndAnswers": "Вопросы и Ответы",
  "notes": "Заметки",
  "typePrivateNotesHere": "Здесь вы можете оставить заметки об игре (доступно только вам)",
  "gameDisplay": "Настройки отображения",
  "pieceAnimation": "Анимация фигур",
  "materialDifference": "Показывать разницу в материале",
  "closeAccount": "Удалить аккаунт",
  "changedMindDoNotCloseAccount": "Я передумал, не удаляйте мой аккаунт",
  "closeAccountExplanation": "Вы действительно желаете удалить свой аккаунт? Удаление аккаунта невозможно отменить. Вы больше не сможете зайти в игру, и ваша страница больше не будет доступна.",
  "thisAccountIsClosed": "Профиль этого пользователя был удалён.",
  "invalidUsernameOrPassword": "Неправильное имя или пароль",
  "incorrectPassword": "Неверный пароль",
  "emailMeALink": "Выслать ссылку",
  "currentPassword": "Текущий пароль",
  "newPassword": "Новый пароль",
  "newPasswordAgain": "Новый пароль (ещё раз)",
  "boardHighlights": "Подсветка (последний ход и шах)",
  "pieceDestinations": "Показывать все возможные ходы",
  "boardCoordinates": "Координаты доски (A-H, 1-8)",
  "moveListWhilePlaying": "Показывать список ходов",
  "pgnPieceNotation": "Запись фигуры в PGN",
  "chessPieceSymbol": "Символ шахматной фигуры",
  "pgnLetter": "Письменная запись PGN (K, Q, R, B, N)",
  "chessClock": "Часы",
  "tenthsOfSeconds": "Десятые секунд",
  "never": "Никогда",
  "whenTimeRemainingLessThanTenSeconds": "Когда остается меньше 10 секунд",
  "horizontalGreenProgressBars": "Горизонтальный зелёный индикатор прогресса",
  "soundWhenTimeGetsCritical": "Звук, когда время подходит к концу",
  "gameBehavior": "Настройки игры",
  "howDoYouMovePieces": "Как вы передвигаете фигуры?",
  "clickTwoSquares": "Нажатием на две клетки",
  "dragPiece": "Перетаскиванием фигуры",
  "bothClicksAndDrag": "Обоими способами",
  "premovesPlayingDuringOpponentTurn": "Предварительный ход (пока ходит противник)",
  "takebacksWithOpponentApproval": "Возвраты (с согласия противника)",
  "promoteToQueenAutomatically": "Пешка превращается в ферзя автоматически",
  "claimDrawOnThreefoldRepetitionAutomatically": "Автоматически запрашивать ничью при трёхкратном повторении хода",
  "privacy": "Приватность",
  "letOtherPlayersFollowYou": "Позволить другим наблюдать за мной",
  "letOtherPlayersChallengeYou": "Другие игроки могут пригласить меня на игру",
  "letOtherPlayersInviteYouToStudy": "Позволить другим игрокам пригласить вас на изучение",
  "sound": "Звук",
  "yourPreferencesHaveBeenSaved": "Ваши настройки сохранены.",
  "none": "Нет",
  "fast": "Быстро",
  "normal": "Нормальная",
  "slow": "Медленно",
  "insideTheBoard": "Внутри поля",
  "outsideTheBoard": "Вне поля",
  "onSlowGames": "В медленных играх",
  "always": "Всегда",
  "inCasualGamesOnly": "Только в играх без рейтинга",
  "whenPremoving": "Когда сделан предварительный ход",
  "whenTimeRemainingLessThanThirtySeconds": "Когда остается меньше 30 секунд",
  "difficultyEasy": "Лёгкий",
  "difficultyNormal": "Нормальный",
  "difficultyHard": "Сложный",
  "xLeftANoteOnY": "%1$s оставил заметку о %2$s",
  "xCompetesInY": "%1$s участвует в %2$s",
  "xAskedY": "%1$s спрашивает %2$s",
  "xAnsweredY": "%1$s ответил %2$s",
  "xCommentedY": "%1$s прокомментировал %2$s",
  "victory": "Победа!",
  "defeat": "Поражение",
  "xVsYinZ": "%1$s с %2$s\nв %3$s",
  "timeline": "Дневник",
  "seeAllTournaments": "Открыть все турниры",
  "starting": "Начинается:",
  "allInformationIsPublicAndOptional": "Вся информация публична и указывается по желанию.",
  "yourCityRegionOrDepartment": "Ваш город, область или округ.",
  "biographyDescription": "Расскажите о себе, что Вы любите в шахматах, Ваши любимые дебюты, партии, игроки...",
  "listBlockedPlayers": "Список заблокированных Вами игроков",
  "human": "Человек",
  "computer": "Компьютер",
  "side": "Сторона",
  "clock": "Часы",
  "unauthorizedError": "Неавторизованный доступ",
  "noInternetConnection": "Нет интернет соединения. Вы можете играть оффлайн из меню.",
  "connectedToLichess": "Сейчас вы подключены к lichess.org",
  "signedOut": "Вы вышли",
  "loginSuccessful": "Вы вошли",
  "playOnTheBoardOffline": "Играйте оффлайн",
  "playOfflineComputer": "Играть с компьютером оффлайн",
  "opponent": "Соперник",
  "learnMenu": "Научиться",
  "community": "Сообщество",
  "tools": "Инструменты",
  "increment": "Прибавка",
  "sharePGN": "Экспорт PGN",
  "playOnline": "Играть онлайн",
  "playOffline": "Играть оффлайн",
  "allowAnalytics": "Разрешить сбор статистики",
  "shareGameURL": "Получить URL игры",
  "error.required": "Заполните это поле",
  "error.email": "Адрес электронной почты является недействительным",
  "error.email_acceptable": "Адрес электронной почты не является приемлемым",
  "error.email_unique": "Адрес электронной почты занят",
  "error.email_different": "Это ваш текущий адрес почты",
  "blindfoldChess": "Играть с завязанными глазами (невидимые фигуры)",
  "zenMode": "Режим Дзен",
  "moveConfirmation": "Подтвердить ход",
  "inCorrespondenceGames": "В игре по переписке",
  "ifRatingIsPlusMinusX": "Если рейтинг ± %s",
  "onlyFriends": "Только друзья",
  "menu": "Меню",
  "castling": "Рокировка",
  "whiteCastlingKingside": "Белые О-О",
  "whiteCastlingQueenside": "Белые О-О-О",
  "blackCastlingKingside": "Черные О-О",
  "blackCastlingQueenside": "Черные О-О-О",
  "tpTimeSpentPlaying": "Времени за игрой: %s",
  "watchGames": "Смотреть игры",
  "tpTimeSpentOnTV": "Времени на TV: %s",
  "watch": "Смотреть",
  "internationalEvents": "Международные события",
  "videoLibrary": "Видеотека",
  "mobileApp": "Мобильное приложение",
  "webmasters": "Разработчикам",
  "contribute": "Помочь Lichess",
  "contact": "Контакты",
  "termsOfService": "Пользовательское соглашение",
  "sourceCode": "Исходный код",
  "simultaneousExhibitions": "Сеанс одновременной игры",
  "host": "Сеансёр",
  "hostColorX": "Цвет организатора: %s",
  "createdSimuls": "Недавно созданные сеансы",
  "hostANewSimul": "Новый сеанс",
  "noSimulFound": "Сеанс не найден",
  "noSimulExplanation": "Этот сеанс одновременной игры не существует.",
  "returnToSimulHomepage": "Вернуться на страницу сеансов",
  "aboutSimul": "Сеансы подразумевают сражение одного игрока с несколькими противниками одновременно.",
  "aboutSimulImage": "Из общего числа в 50 игр Фишер выиграл 47, 2 сыграл вничью и проиграл 1.",
  "aboutSimulRealLife": "Идея повторяет принятую в живых шахматах концепцию, когда дающий сеанс одновременной игры перемещается от стола к столу, чтобы совершить один ход.",
  "aboutSimulRules": "Старт - каждый игрок начинает игру с сеансёром, получающим белый цвет. Сеанс закончится, когда все партии будут сыграны.",
  "aboutSimulSettings": "Сеансы одновременной игры не предполагают партий на рейтинг. Переигровки, отмены ходов и \"добавки времени\\\" запрещены.",
  "create": "Создать",
  "whenCreateSimul": "Когда вы создаёте Сеанс, вы играете против нескольких игроков одновременно.",
  "simulVariantsHint": "Если вы выберете несколько вариантов, то каждый игрок должен будет выбрать желаемый режим.",
  "simulClockHint": "Установка Часов Фишера. Чем больше игроков играет против вас, тем больше времени вам может понадобится.",
  "simulAddExtraTime": "Вы можете добавить дополнительное время к своему таймеру для большей форы.",
  "simulHostExtraTime": "Дополнительное время сеансёра",
  "lichessTournaments": "Турниры Lichess",
  "tournamentFAQ": "ЧАВо турнира Арена",
  "tournamentOfficial": "Официальный",
  "timeBeforeTournamentStarts": "Время до начала турнира",
  "averageCentipawnLoss": "В среднем потеряно сотых пешки",
  "keyboardShortcuts": "Горячие клавиши",
  "keyMoveBackwardOrForward": "ход назад/вперед",
  "keyGoToStartOrEnd": "в начало/конец",
  "keyShowOrHideComments": "показать/скрыть комментарии",
  "keyEnterOrExitVariation": "ввести/закрыть вариант",
  "newTournament": "Новый турнир",
  "tournamentHomeTitle": "Турнир по шахматным вариациям с различными контролем времени",
  "tournamentHomeDescription": "Играйте в шахматных турнирах в быстрые шахматы! Присоединяйтесь к запланированному турнира или создайте собственный. Пуля, блиц, классика, шахматы Фишера, король в центре, три шаха и другие доступные варианты бесконечного шахматного удовольствия.",
  "tournamentNotFound": "Турнир не найден",
  "tournamentDoesNotExist": "Турнира не существует.",
  "tournamentMayHaveBeenCanceled": "Может быть отменен, в случае если все игроки покинули турнир до его старта.",
  "returnToTournamentsHomepage": "Вернуться на страницу турниров",
  "weeklyPerfTypeRatingDistribution": "Распределение по рейтингу %s за месяц",
  "yourPerfTypeRatingIsRating": "Ваш %1$s рейтинг %2$s.",
  "youAreBetterThanPercentOfPerfTypePlayers": "Вы сильнее %1$s игроков в %2$s.",
  "youDoNotHaveAnEstablishedPerfTypeRating": "Ваш %s рейтинг ещё не определен.",
  "checkYourEmail": "Подтвердите свой Email",
  "weHaveSentYouAnEmailClickTheLink": "Мы отправили Вам письмо. Нажмите на ссылку в нём для активации аккаунта.",
  "ifYouDoNotSeeTheEmailCheckOtherPlaces": "Если письмо не приходит, проверьте папку для спама и другие места, куда оно могло попасть.",
  "areYouSureYouEvenRegisteredYourEmailOnLichess": "Вы уверены, что указывали свой Email на Lichess?",
  "itWasNotRequiredForYourRegistration": "Он не был обязателен при регистрации.",
  "weHaveSentYouAnEmailTo": "Мы отправили письмо по адресу %s. Нажмите на ссылку в нём для сброса пароля.",
  "byRegisteringYouAgreeToBeBoundByOur": "Регистрируясь, вы берёте на себя обязанность соблюдать наше %s",
  "networkLagBetweenYouAndLichess": "Задержка сети между Вами и серверами lichess",
  "timeToProcessAMoveOnLichessServer": "Время обработки хода на сервере",
  "downloadAnnotated": "Скачать с аннотациями",
  "downloadRaw": "Скачать без аннотаций",
  "downloadImported": "Скачать в изначальном виде",
  "printFriendlyPDF": "Скачать в PDF",
  "crosstable": "Счёт",
  "youCanAlsoScrollOverTheBoardToMoveInTheGame": "Можно использовать колёсико над доской для перемотки ходов.",
  "analysisShapesHowTo": "Чтобы рисовать кружки и стрелки на доске, используйте Shift+левую кнопку мыши или правую кнопку.",
  "confirmResignationAndDrawOffers": "Подтверждать возврат хода и предложения ничей",
  "inputMovesWithTheKeyboard": "Вводить ходы с помощью клавиатуры",
  "letOtherPlayersMessageYou": "Позволить присылать Вам сообщения",
  "castleByMovingTheKingTwoSquaresOrOntoTheRook": "Рокировать перемещением короля на две клетки или нажатием на ладью",
  "castleByMovingTwoSquares": "Рокировать перемещением короля на две клетки",
  "castleByMovingOntoTheRook": "Рокировать нажатием на ладью",
  "shareYourInsightsData": "Поделиться статистикой",
  "withNobody": "Ни с кем",
  "withFriends": "С друзьями",
  "withEverybody": "Со всеми",
  "youHaveAlreadyRegisteredTheEmail": "Вы уже активировали этот адрес: %s",
  "kidMode": "Детский режим",
  "kidModeExplanation": "Это для безопасности. В детском режиме отключены все коммуникации на сайте. Включите это для ваших детей и школьников, чтобы защитить их от других пользователей Интернета.",
  "inKidModeTheLichessLogoGetsIconX": "В детском режиме логотип lichess получает значок в виде %s, так что вы знаете, что ваши дети находятся в безопасности.",
  "enableKidMode": "Включить детский режим",
  "disableKidMode": "Отключить детский режим",
  "security": "Безопасность",
  "thisIsAListOfDevicesThatHaveLoggedIntoYourAccount": "Это список устройств, с которых входили в ваш аккаунт. Отмените любые подозрительные сеансы.",
  "alternativelyYouCanX": "В качестве альтернативы вы можете %s.",
  "revokeAllSessions": "отменить все сессии",
  "playChessEverywhere": "Играйте в шахматы везде",
  "asFreeAsLichess": "Бесплатно",
  "builtForTheLoveOfChessNotMoney": "С любовью к шахматам, а не деньгам",
  "everybodyGetsAllFeaturesForFree": "Никакой оплаты",
  "zeroAdvertisement": "Без рекламы",
  "fullFeatured": "Полнофункционально",
  "phoneAndTablet": "На телефоне и планшете",
  "bulletBlitzClassical": "Игра в реальном времени",
  "correspondenceChess": "Игра по переписке",
  "onlineAndOfflinePlay": "Онлайн и оффлайн",
  "correspondenceAndUnlimited": "По переписке и без ограничения времени",
  "viewTheSolution": "Посмотреть решение",
  "followAndChallengeFriends": "Подписаться и вызвать на игру друзей",
  "gameAnalysis": "Анализ игры",
  "xHostsY": "%1$s создал %2$s",
  "xJoinsY": "%1$s присоединился к %2$s",
  "xLikesY": "%1$s отметил %2$s как понравившееся",
  "quickPairing": "Быстрый старт",
  "lobby": "Лобби",
  "anonymous": "Анонимно",
  "yourScore": "Ваш результат: %s",
  "language": "Язык",
  "background": "Фон",
  "light": "Светлая",
  "dark": "Тёмная",
  "transparent": "Прозрачная",
  "backgroundImageUrl": "URL фонового изображения:",
  "boardGeometry": "Геометрия доски",
  "boardTheme": "Оформление доски",
  "boardSize": "Размер доски",
  "pieceSet": "Набор фигур",
  "embedInYourWebsite": "Встроить в свой сайт",
  "usernameAlreadyUsed": "Данное имя пользователя занято. Пожалуйста, попробуйте другое.",
  "usernameInvalid": "Некорректное имя пользователя. Пожалуйста, используйте только буквы, цифры, подчеркивания и тире.",
  "usernameStartNoNumber": "Имя пользователя не должно начинаться с цифры.",
  "usernameUnacceptable": "Это имя пользователя не является приемлемым.",
  "directlySupportLichess": "Поддерживает Lichess",
  "playChessInStyle": "В стиле шахмат",
  "chessBasics": "Шахматные основы",
  "coaches": "Тренеры",
  "invalidPgn": "Некорректный PGN",
  "invalidFen": "Некорректный FEN",
  "error.minLength": "Минимальная длина — %s",
  "error.maxLength": "Максимальная длина — %s",
  "error.min": "Значение должно быть больше либо равным %s",
  "error.max": "Значение должно быть меньше либо равным %s",
  "error.unknown": "Ошибка",
  "custom": "Своя игра",
  "notifications": "Уведомления",
  "challenges": "Вызовы",
  "perfRatingX": "Рейтинг: %s",
  "mateInXHalfMoves:one": "Мат в %s ходе сторон",
  "mateInXHalfMoves:few": "Мат в %s ходах сторон",
  "mateInXHalfMoves:other": "Мат в %s ходах сторон",
  "nextCaptureOrPawnMoveInXHalfMoves:one": "Следующий захват или ход пешкой в %s ходе сторон (Дистанция до 50-го хода)",
  "nextCaptureOrPawnMoveInXHalfMoves:few": "Следующий захват или ход пешкой в %s ходах сторон (Дистанция до 50-го хода)",
  "nextCaptureOrPawnMoveInXHalfMoves:other": "Следующий захват или ход пешкой в %s ходах сторон (Дистанция до 50-го хода)",
  "nbPlayers:one": "%s игрок онлайн",
  "nbPlayers:few": "%s игрока онлайн",
  "nbPlayers:other": "%s игроков онлайн",
  "nbGames:one": "%s игра",
  "nbGames:few": "%s игр",
  "nbGames:other": "%s игр",
  "nbBookmarks:one": "%s закладка",
  "nbBookmarks:few": "%s закладки",
  "nbBookmarks:other": "%s закладок",
  "nbDays:one": "%s день",
  "nbDays:few": "%s дня",
  "nbDays:other": "%s дней",
  "nbHours:one": "%s час",
  "nbHours:few": "%s часа",
  "nbHours:other": "%s часов",
  "nbPuzzles:one": "%s пазл",
  "nbPuzzles:few": "%s пазла",
  "nbPuzzles:other": "%s пазлов",
  "nbGamesWithYou:one": "%s партия с вами",
  "nbGamesWithYou:few": "%s партии с вами",
  "nbGamesWithYou:other": "%s партий с вами",
  "nbRated:one": "%s по рейтингу",
  "nbRated:few": "%s по рейтингу",
  "nbRated:other": "%s по рейтингу",
  "nbWins:one": "%s выиграно",
  "nbWins:few": "%s выиграно",
  "nbWins:other": "%s выиграно",
  "nbLosses:one": "%s проиграно",
  "nbLosses:few": "%s проиграно",
  "nbLosses:other": "%s проиграно",
  "nbDraws:one": "%s сыграно вничью",
  "nbDraws:few": "%s сыграно вничью",
  "nbDraws:other": "%s сыграно вничью",
  "nbPlaying:one": "%s играет",
  "nbPlaying:few": "%s играет",
  "nbPlaying:other": "%s играет",
  "giveNbSeconds:one": "Дать %s секунду",
  "giveNbSeconds:few": "Дать %s секунды",
  "giveNbSeconds:other": "Дать %s секунд",
  "nbMembers:one": "%s участник",
  "nbMembers:few": "%s участника",
  "nbMembers:other": "%s участников",
  "nbImportedGames:one": "%s Импортированная игра",
  "nbImportedGames:few": "%s Импортированные игры",
  "nbImportedGames:other": "%s Импортированных игр",
  "nbFollowers:one": "%s подписчик",
  "nbFollowers:few": "%s подписчика",
  "nbFollowers:other": "%s подписчиков",
  "nbFollowing:one": "подписан на %s игрока",
  "nbFollowing:few": "%s подписан",
  "nbFollowing:other": "подписан на %s игроков",
  "lessThanNbMinutes:one": "Менее %s минуты",
  "lessThanNbMinutes:few": "Менее %s минут",
  "lessThanNbMinutes:other": "Менее %s минут",
  "playedXTimes:one": "Решено %s раз",
  "playedXTimes:few": "Решено %s раза",
  "playedXTimes:other": "Решено %s раз",
  "youHaveNbSecondsToMakeYourFirstMove:one": "У Вас %s секунда, чтобы сделать свой первый ход!",
  "youHaveNbSecondsToMakeYourFirstMove:few": "У Вас %s секунды, чтобы сделать свой первый ход!",
  "youHaveNbSecondsToMakeYourFirstMove:other": "У Вас %s секунд, чтобы сделать свой первый ход!",
  "nbGamesInPlay:one": "%s ведется партий",
  "nbGamesInPlay:few": "%s ведется партий",
  "nbGamesInPlay:other": "%s ведется партий",
  "maximumNbCharacters:one": "Максимум: %s символов.",
  "maximumNbCharacters:few": "Максимум: %s символов.",
  "maximumNbCharacters:other": "Максимум: %s символов.",
  "blocks:one": "%s заблокировано",
  "blocks:few": "%s заблокировано",
  "blocks:other": "%s заблокировано",
  "nbForumPosts:one": "%s постов на форуме",
  "nbForumPosts:few": "%s постов на форуме",
  "nbForumPosts:other": "%s постов на форуме",
  "nbPerfTypePlayersThisWeek:one": "%1$s %2$s игроков на этой неделе.",
  "nbPerfTypePlayersThisWeek:few": "%1$s %2$s игроков на этой неделе.",
  "nbPerfTypePlayersThisWeek:other": "%1$s %2$s игроков на этой неделе.",
  "availableInNbLanguages:one": "Доступно на %s языке!",
  "availableInNbLanguages:few": "Доступно на %s языках!",
  "availableInNbLanguages:other": "Доступно на %s языках!"
}